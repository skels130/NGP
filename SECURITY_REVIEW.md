# Security and Code Review - Config_Gen
...
**Deployment Architecture**: Config_Gen operates behind the NetSapiens NDP server as a proxied backend service. Gateways request configs from NDP at `/gateway/{MAC}.cfg`, which proxies to Config_Gen. See **DEPLOYMENT.md** for proxy configuration.
...
**Recommended Fix**: Create `/path/to/Config_Gen/.htaccess`:
```apache
...
server {
    ...
    root /var/www/config-gen/public;
...
**Architecture**:
```
Internet → NDP Server (Public) → Config_Gen (Private Network) → ns-api
           DMZ/Public IP           Backend/Private IP
```

**Security Benefits**:
1. **Defense in Depth**: Config_Gen server never exposed to public internet
2. **IP Whitelisting**: Firewall restricts Config_Gen to only accept NDP server connections
3. **Single Entry Point**: Only NDP server is public-facing, reducing attack surface
4. **Network Segmentation**: Config_Gen can be on internal VLAN/subnet

**Recommended Firewall Rules** (Config_Gen server):
```bash
