# Deployment Guide - NGP (NetSapiens Gateway Provisioning)
...
Create Apache virtual host: `/etc/apache2/sites-available/ngp.conf`
...
<VirtualHost *:443>
    ...
    # Logging
    ErrorLog ${APACHE_LOG_DIR}/ngp-error.log
    CustomLog ${APACHE_LOG_DIR}/ngp-access.log combined
</VirtualHost>

# HTTP to HTTPS redirect
<VirtualHost *:80>
    ServerName ngp.example.com
    Redirect permanent / https://ngp.example.com/
</VirtualHost>
```
...
<Location /gateway>
    ProxyPass https://ngp.example.com/gateway
    ProxyPassReverse https://ngp.example.com/gateway
...
```nginx
location /gateway/ {
    proxy_pass https://ngp.example.com/gateway/;
...
<Location /gateway>
    RewriteEngine On
    RewriteRule ^/gateway/(.*)$ https://ngp.example.com/$1 [P,L]
    ProxyPassReverse https://ngp.example.com/
</Location>
```
...
location /gateway/ {
    ...
    proxy_pass https://ngp.example.com;
}
```
...
curl https://ngp.example.com/C074AD893044.cfg

# Should return config (with valid credentials)
curl -u username:password https://ngp.example.com/C074AD893044.cfg
```
...
curl https://ngp.example.com/config/config.php
curl https://ngp.example.com/src/NsApiClient.php
curl https://ngp.example.com/../config/config.php
```
...
```bash
# On NGP server
tail -f /var/www/ngp/logs/ngp.log
...
2. Check NDP server can reach NGP server:
   ```bash
   # From NDP server
   curl -I https://ngp.example.com/C074AD893044.cfg
   ```
...
3. Review NGP logs:
   ```bash
   grep "Authentication failed" /var/www/ngp/logs/ngp.log
   ```
...
2. Check PHP error logs:
   ```bash
   tail -f /var/log/apache2/ngp-error.log
   ```
...
2. Check template directory exists:
   ```bash
   ls -la /var/www/ngp/templates/{brand}/{model}/
   ```
...
|----------------|--------------|-------------|---------------------|
| `/gateway/{MAC}.cfg` | `/gateway/{MAC}.cfg` | → `https://ngp.example.com/gateway/{MAC}.cfg` | `/gateway/{MAC}.cfg` ✓ |
| `/gateway/{MAC}.cfg` | `/gateway/{MAC}.cfg` | → `https://ngp.example.com/{MAC}.cfg` | `/{MAC}.cfg` ✓ |
...
1. Review logs: `/var/www/ngp/logs/ngp.log`
